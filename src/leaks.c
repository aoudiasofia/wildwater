...
// ==================================================================
// GESTION DU GRAPHE (NOEUDS ET ARÊTES)
// ==================================================================
...
Liaison* ajouterLiaison(Station* parent, Station* enfant, double fuite) {
    ...
    // Insertion en tête de liste (plus rapide O(1))
...
// ==================================================================
// GESTION DE L'INDEX (AVL TECHNIQUE)
// ==================================================================
// Note: On réutilise la logique AVL standard (rotations, équilibre)
// Pour gagner du temps, on suppose que les fonctions hauteur/max sont accessibles via wildwater.h
// Mais comme elles sont définies dans avl.c, nous devons les redéfinir ici ou les rendre globales.
// Pour éviter les conflits de linkage, on les rend "static" ici ou on les copie.
// Copions les utilitaires de base pour être autonomes.
...
NoeudIndex* insererIndex(NoeudIndex* noeud, char* id, Station* station) {
    ...
    if (bal > 1 && strcmp(id, noeud->gauche->id) < 0) return rotD_index(noeud);
    if (bal < -1 && strcmp(id, noeud->droite->id) > 0) return rotG_index(noeud);
...
Station* rechercherStation(NoeudIndex* racine, char* id) {
    ...
    if (cmp == 0) return racine->station;
    if (cmp < 0) return rechercherStation(racine->gauche, id);
    return rechercherStation(racine->droite, id);
}

// ==================================================================
// LOGIQUE MÉTIER : CALCUL DES FUITES
// ==================================================================

/*
 * Algorithme Récursif :
 * 1. On reçoit un volume d'eau entrant dans la station actuelle.
 * 2. On compte combien on a d'enfants (liaisons sortantes).
 * 3. On divise le volume équitablement : Vol_Par_Tuyau = Volume / Nb_Enfants.
 * 4. Pour chaque tuyau :
...
double calculerFuites(Station* depart, double volume_initial) {
    if (depart == NULL || volume_initial <= 0) return 0.0;
    ...
    if (nb_enfants == 0) return 0.0; // Fin de ligne (Usager), pas de fuite avale
...
void libererGraphe(NoeudIndex* indexRacine) {
    if (indexRacine != NULL) {
        ...
        // Libération de la Station associée (Graphe)
        // Attention : Une station est pointée par l'index. On la libère ici.
        ...
        if (s != NULL) {
            ...
            while (l != NULL) {
                Liaison* tmp = l;
                l = l->suivant;
                free(tmp); // On libère l'arête, mais pas le noeud enfant (il sera libéré via l'index)
            }
            free(s->id);
            free(s);
        }

        // Libération du noeud d'index
        free(indexRacine->id);
        free(indexRacine);
    }
}